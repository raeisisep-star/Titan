// Ultra Simple Settings Module for Testing
class UltraSimpleSettingsModule {
    constructor() {
        this.currentTab = 'general';
    }

    async initialize() {
        console.log('✅ Ultra Simple Settings initialized');
    }
        ...
        return '<div class="space-y-6">' +
            ...
            '<div class="bg-gray-800 rounded-lg border border-gray-700">' +
                ...
                '<div class="flex border-b border-gray-700 overflow-x-auto">' +
                    '<button onclick="settingsModule.switchTab(\'general\')" class="settings-tab px-6 py-4 text-sm font-medium whitespace-nowrap flex items-center gap-2 text-blue-400 border-b-2 border-blue-400">' +
                        '<i class="fas fa-cog"></i>عمومی' +
                    '</button>' +
                    '<button onclick="settingsModule.switchTab(\'ai\')" class="settings-tab px-6 py-4 text-sm font-medium whitespace-nowrap flex items-center gap-2 text-gray-400 hover:text-white">' +
                        '<i class="fas fa-robot"></i>هوش مصنوعی' +
                    '</button>' +
                    '<button onclick="settingsModule.switchTab(\'trading\')" class="settings-tab px-6 py-4 text-sm font-medium whitespace-nowrap flex items-center gap-2 text-gray-400 hover:text-white">' +
                        '<i class="fas fa-chart-line"></i>معاملات' +
                    '</button>' +
                    '<button onclick="settingsModule.switchTab(\'system\')" class="settings-tab px-6 py-4 text-sm font-medium whitespace-nowrap flex items-center gap-2 text-gray-400 hover:text-white">' +
    ...
    switchTab(tabName) {
        ...
        const selector = "settingsModule.switchTab('" + tabName + "')";
        const activeTab = document.querySelector('[onclick="' + selector + '"]');
        if (activeTab) {
...
window.TitanModules.SettingsModule = UltraSimpleSettingsModule;
