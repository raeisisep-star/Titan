...
// Real-time log monitoring and analysis for TITAN backend
// Phase 3.3: Central Monitoring & Alert Dashboard
// 
// Features:
...
class LogsDashboardManager {
    ...
    async init() {
        ...
        try {
            console.log('🔧 Initializing Logs Dashboard Manager...');
            
            // Check if we're on the logs dashboard page
            const container = document.getElementById('logsDashboardContainer');
            if (!container) {
                console.log('ℹ️ Logs dashboard container not found, skipping initialization');
                return;
            }
            ...
            console.log('✅ Logs Dashboard Manager initialized successfully');
            
        } catch (error) {
            console.error('❌ Failed to initialize Logs Dashboard Manager:', error);
            this.showError('خطا در بارگذاری داشبورد لاگ‌ها: ' + error.message);
        }
    }
    ...
    renderUI() {
        ...
        container.innerHTML = `
            <div class="logs-dashboard p-6 space-y-6">
                <!-- Header -->
                <div class="flex justify-between items-center">
                    <div>
                        <h2 class="text-3xl font-bold text-white flex items-center">
                            <i class="fas fa-list-alt ml-3"></i>
                            داشبورد لاگ‌های سیستم
                        </h2>
                        <p class="text-gray-400 mt-2">
                            مشاهده و تحلیل لاگ‌های real-time سیستم TITAN
                        </p>
                    </div>
                    <div class="flex items-center space-x-3 space-x-reverse">
                        <button id="refreshButton" class="btn-primary px-4 py-2 rounded-lg">
                            <i class="fas fa-sync-alt ml-2"></i>
                            بروزرسانی
                        </button>
                        <button id="toggleAutoRefresh" class="btn-secondary px-4 py-2 rounded-lg">
                            <i class="fas fa-pause ml-2"></i>
                            <span id="autoRefreshText">توقف خودکار</span>
                        </button>
                    </div>
                </div>
                
                <!-- Stats Cards -->
                <div id="statsCards" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-gray-800 p-4 rounded-lg">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select id="levelFilter" class="w-full bg-gray-700 text-white px-3 py-2 rounded-lg border border-gray-600 focus:border-blue-500 focus:outline-none">
                        ...
                        <div>
                            ...
                            <input type="text" id="searchInput" placeholder="جست‌وجو در پیام‌ها..." class="w-full bg-gray-700 text-white px-3 py-2 rounded-lg border border-gray-600 focus:border-blue-500 focus:outline-none">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                            ...
                            <select id="limitFilter" class="w-full bg-gray-700 text-white px-3 py-2 rounded-lg border border-gray-600 focus:border-blue-500 focus:outline-none">
                    ...
                    <div class="mt-3 text-sm text-gray-400 flex items-center">
                        ...
                        آخرین بروزرسانی: <span id="lastRefreshTime" class="mr-2 font-mono">-</span>
                    </div>
                </div>
                ...
                <div class="border-b border-gray-700">
                    <div class="flex space-x-4 space-x-reverse">
                        <button class="tab-button active px-4 py-2 text-white border-b-2 border-blue-500" data-tab="logs">
                            <i class="fas fa-file-alt ml-2"></i>
                            لاگ‌های اخیر
                        </button>
                        <button class="tab-button px-4 py-2 text-gray-400 border-b-2 border-transparent hover:text-white" data-tab="errors">
                            <i class="fas fa-exclamation-circle ml-2"></i>
                            خطاها
                            <span id="errorCount" class="mr-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full">0</span>
                        </button>
                    </div>
                </div>
                ...
                <div id="logsTab" class="tab-content">
                    <div class="bg-gray-800 rounded-lg p-4">
                        <div id="logsContainer" class="space-y-2 max-h-screen overflow-y-auto">
                ...
                <div id="errorsTab" class="tab-content hidden">
                    <div class="bg-gray-800 rounded-lg p-4">
                        <div id="errorsContainer" class="space-y-2 max-h-screen overflow-y-auto">
    ...
    renderStats() {
        ...
        container.innerHTML = `
            <div class="bg-gradient-to-br from-red-600 to-red-800 p-4 rounded-lg text-white">
            ...
            <div class="bg-gradient-to-br from-yellow-600 to-yellow-800 p-4 rounded-lg text-white">
                <div class="flex items-center justify-between">
                    <div>
            ...
            <div class="bg-gradient-to-br from-blue-600 to-blue-800 p-4 rounded-lg text-white">
                <div class="flex items-center justify-between">
                    <div>
            ...
            <div class="bg-gradient-to-br from-green-600 to-green-800 p-4 rounded-lg text-white">
                <div class="flex items-center justify-between">
                    <div>
    ...
    renderLogItem(log) {
        ...
        return `
            <div class="log-item bg-gray-900 p-3 rounded-lg border-r-4" style="border-color: ${config.color}">
    ...
    renderErrors() {
        ...
        const errorsHTML = this.errors.map(error => `
            <div class="error-item bg-red-900/20 p-3 rounded-lg border border-red-500/30">
    ...
    bindEvents() {
        ...
        if (limitFilter) {
            limitFilter.addEventListener('change', (e) => {
                this.currentFilter.limit = parseInt(e.target.value);
                this.handleRefresh();
            });
        }
    ...
    startAutoRefresh() {
        ...
        this.autoRefreshInterval = setInterval(() => {
            console.log('🔄 Auto-refreshing logs dashboard...');
            this.fetchAllData();
    ...
    destroy() {
        this.stopAutoRefresh();
        this.isInitialized = false;
        console.log('🗑️ Logs Dashboard Manager destroyed');
    }
}

// Initialize globally
window.LogsDashboardManager = LogsDashboardManager;

// Auto-initialize if DOM is ready
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        if (document.getElementById('logsDashboardContainer')) {
            window.logsDashboard = new LogsDashboardManager();
        }
    });
} else {
    if (document.getElementById('logsDashboardContainer')) {
