...
class TitanApp {
    ...
    async verifyToken(token) {
        try {
            ...
            if (response.status === 200 && response.data.ok && response.data.authenticated) {
                this.currentUser = response.data.user;
                this.isDemo = false; // Real authenticated user
                console.log('✅ Token verified, user authenticated:', response.data.user.username);
                this.showMainApp();
            } else {
                // Token invalid or not authenticated
                console.log('❌ Token verification failed (401), showing login screen');
                localStorage.removeItem('titan_auth_token');
                delete axios.defaults.headers.common['Authorization'];
                this.isDemo = true; // Fall back to demo mode
                this.showLoginScreen();
            }
        } catch (error) {
...
💡 برای شروع، بگویید: "نمایش پورتفولیو" یا "کمک"`;
    ...
    async renderPortfolioSummaryWidget(widget) {
        try {
            ...
            const fetchResponse = await fetch('/dashboard/comprehensive', {
                headers: {
    ...
    async renderMarketOverviewWidget(widget) {
        try {
            ...
            const response = await fetch('/dashboard/comprehensive', {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('titan_auth_token')}`,
                    'Content-Type': 'application/json'
                }
    ...
    async initializeWidgetChart(widget) {
        ...
        try {
            ...
            const response = await fetch('/portfolio/advanced', {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('titan_auth_token')}`,
                    'Content-Type': 'application/json'
                }
    ...
    async getPerformanceHistory() {
        ...
        const dashResponse = await fetch('/dashboard/comprehensive', {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('titan_auth_token')}`,
                    'Content-Type': 'application/json'
                }
    ...
    async initializeModeToggle() {
        try {
            ...
            try {
                ...
                console.warn('⚠️ Mode API unavailable, using saved mode:', savedMode);
