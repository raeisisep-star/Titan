...
class SettingsModule {
    ...
    // Helper method for tab classes
    getTabClass(tabName) {
        const baseClass = "settings-tab px-6 py-4 text-sm font-medium whitespace-nowrap flex items-center gap-2";
        const activeClass = "text-blue-400 border-b-2 border-blue-400";
        const inactiveClass = "text-gray-400 hover:text-white";
        return `${baseClass} ${this.currentTab === tabName ? activeClass : inactiveClass}`;
    }
    ...
    getSystemTab() {
        ...
        <div class="space-y-6">
            ...
            <div class="bg-gray-900 rounded-lg p-4">
                <div class="flex items-center justify-between mb-4">
                    <h4 class="text-lg font-semibold text-white">📋 لاگ‌های سیستم</h4>
                    <button onclick="settingsModule.openFullLogsViewer()" class="bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded text-white text-sm">
                        <i class="fas fa-external-link-alt ml-2"></i>مشاهده کامل
                    </button>
                </div>
                <div id="system-logs-preview" class="space-y-2">
                    <p class="text-gray-400 text-sm">در حال بارگذاری لاگ‌های سیستم...</p>
                </div>
            </div>
        </div>`;
    }
    ...
    async saveNewWallet() {
        ...
        try {
            ...
            button.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>در حال اضافه کردن...';
            button.disabled = true;

            // Call real API
            const response = await fetch(`${window.location.origin}/api/wallets/connect`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token') || 'demo_token_' + Date.now()}`
                },
                body: JSON.stringify({
                    name: name,
                    type: isCold ? 'Cold Storage' : 'Trading',
                    exchange: type,
                    address: address,
                    apiKey: address, // Using address as API key for now
            ...
            if (result.success) {
                ...
                throw new Error(result.error || 'خطا در اضافه کردن کیف پول');
            }

        } catch (error) {
            console.error('Error adding wallet:', error);
            this.showToast('❌ خطا در اضافه کردن کیف پول: ' + error.message, 'error');
        } finally {
            // Reset button state
            const button = event.target;
            if (button) {
                button.innerHTML = 'افزودن';
    ...
    processWalletImport() {
        ...
        this.performWalletImport(method, data);
    ...
    async refreshHotWallets() {
        ...
        try {
            // Call real API
            const response = await fetch(`${window.location.origin}/api/wallets`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token') || 'demo_token_' + Date.now()}`
                }
            ...
            if (result.success && result.data && result.data.wallets) {
                const wallets = result.data.wallets.filter(w => w.type !== 'Cold Storage');
                
                if (wallets.length === 0) {
                    hotWalletsList.innerHTML = `
                        <div class="text-center p-8 text-gray-400">
                            <i class="fas fa-wallet text-4xl mb-4"></i>
                            <p>هیچ کیف پول گرمی یافت نشد</p>
                        </div>
                    `;
                    return;
                }

                hotWalletsList.innerHTML = wallets.map(wallet => `
                    <div class="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center">
                                <i class="fas fa-wallet text-white text-sm"></i>
                            </div>
                            <div>
                                <div class="font-medium text-white">${wallet.name}</div>
                                <div class="text-xs text-gray-400">${wallet.address || wallet.exchange}</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <div class="text-right">
                                <div class="text-sm font-medium text-white">$${wallet.balance.toLocaleString()}</div>
                                <div class="text-xs text-green-400">${wallet.currency}</div>
                            </div>
                            <div class="w-2 h-2 ${wallet.status === 'Active' ? 'bg-green-400' : 'bg-red-400'} rounded-full" title="${wallet.status}"></div>
                        </div>
                    </div>
                `).join('');
                ...
                throw new Error(result.error || 'خطا در دریافت اطلاعات کیف پول‌ها');
            }
            ...
            hotWalletsList.innerHTML = `
                <div class="text-center p-8 text-red-400">
                    ...
                    <p>خطا در بارگذاری کیف پول‌ها</p>
                    <button onclick="settingsModule.refreshHotWallets()" class="mt-3 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                        تلاش مجدد
                    </button>
                </div>
            `;
    ...
    async performWalletImport(method, data) {
        try {
            this.showToast('در حال وارد کردن کیف پول...', 'info');

            const response = await fetch(`${window.location.origin}/api/wallets/import`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token') || 'demo_token_' + Date.now()}`
            ...
            if (result.success) {
                this.showToast(`✅ ${result.data.imported} کیف پول با موفقیت وارد شد`, 'success');
                document.querySelector('.fixed').remove();
                this.refreshHotWallets();
            } else {
                throw new Error(result.error || 'خطا در واردات کیف پول');
            }

        } catch (error) {
            console.error('Wallet import error:', error);
            this.showToast('❌ خطا در واردات: ' + error.message, 'error');
        }
    }

    updateWalletStats(data) {
        // Update total wallets count
        const totalWalletsElement = document.getElementById('total-wallets');
        if (totalWalletsElement && data.wallets) {
            totalWalletsElement.textContent = data.wallets.length;
        }

        // Update total balance
        const totalBalanceElement = document.getElementById('total-balance');
        if (totalBalanceElement && data.totalBalance) {
            totalBalanceElement.textContent = `$${data.totalBalance.toLocaleString()}`;
        }

        // Update asset types
        const assetTypesElement = document.getElementById('asset-types');
        if (assetTypesElement && data.wallets) {
            const uniqueAssets = [...new Set(data.wallets.map(w => w.currency))];
            assetTypesElement.textContent = uniqueAssets.length;
        }

        // Update cold wallets count
        const coldWalletsElement = document.getElementById('cold-wallets');
        if (coldWalletsElement && data.wallets) {
            const coldWallets = data.wallets.filter(w => w.type === 'Cold Storage');
            coldWalletsElement.textContent = coldWallets.length;
        }
    }
    ...
    async testColdWallets() {
        this.showToast('در حال تست اتصال کلد والت‌ها...', 'info');
        
        try {
            // Call real API
            const response = await fetch(`${window.location.origin}/api/wallets/cold-wallet/test`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token') || 'demo_token_' + Date.now()}`
                }
            ...
            if (result.success) {
                const results = [
                    `✅ اتصال اصلی: ${result.data.primaryConnection ? 'موفق' : 'ناموفق'}`,
                    `📡 بکاپ: ${result.data.backupConnection ? 'آماده' : 'غیرفعال'}`,
                    `🔒 امنیت: ${result.data.securityLevel}/100`,
                    `⚡ پینگ: ${result.data.responseTime}ms`,
                    `💰 تراکنش‌های امروز: ${result.data.todayTransfers}`,
                    `📊 درصد امنیت: ${result.data.safetyPercentage}%`
                ];
...
پایان گزارش
    ...
    openFullLogsViewer() {
        ...
        if (typeof app !== 'undefined' && typeof app.loadModule === 'function') {
            ...
            console.error('App module loader not available');
            alert('خطا در بارگذاری ماژول لاگ‌ها');
        }
    }

    async loadSystemLogsPreview() {
        const previewContainer = document.getElementById('system-logs-preview');
        ...
        try {
            ...
            if (data.success && data.data && data.data.logs && data.data.logs.length > 0) {
                ...
                previewContainer.innerHTML = logsHtml + `
                    <div class="text-center pt-2">
                        <button onclick="settingsModule.openFullLogsViewer()" class="text-blue-400 hover:text-blue-300 text-sm">
                            مشاهده همه لاگ‌ها →
                        </button>
                    </div>
                `;
            } else {
                previewContainer.innerHTML = '<p class="text-gray-500 text-sm">هیچ لاگی یافت نشد</p>';
            }
        } catch (error) {
            console.error('Error loading logs preview:', error);
            previewContainer.innerHTML = `
                <p class="text-red-400 text-sm">خطا در بارگذاری لاگ‌ها</p>
                <button onclick="settingsModule.loadSystemLogsPreview()" class="text-blue-400 hover:text-blue-300 text-sm mt-2">
                    تلاش مجدد
                </button>
            `;
        }
    ...
    refreshCurrentTab() {
        ...
        if (content) {
            ...
            // Load logs preview if on system tab
            if (this.currentTab === 'system') {
                setTimeout(() => this.loadSystemLogsPreview(), 100);
            }
        }
    }
}
